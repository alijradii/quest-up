...
export function QuestStatistics({ quests }: QuestStatisticsProps) {
  const stats = {
    ...
    totalXP: quests.filter((q) => q.status === "completed").reduce((sum, q) => sum + q.xp, 0),
  }

  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5 mb-6">
      <StatCard title="Total Quests" value={stats.total} icon={<Target className="h-4 w-4 text-muted-foreground" />} />
      <StatCard
        title="Completed"
